...
read_dev_env_value() {
  read_env_value_from_file "$DEV_ENV_FILE" "$1"
}
...
USAGE
  ...
  if [[ -z "${current_db_user:-}" ]]; then
    current_db_user="$(read_dev_env_value POSTGRES_USER || true)"
  fi
  if [[ -z "${current_db_password:-}" ]]; then
    current_db_password="$(read_dev_env_value POSTGRES_PASSWORD || true)"
  fi
  if [[ -z "${current_minio_root_user:-}" ]]; then
    current_minio_root_user="$(read_dev_env_value MINIO_ROOT_USER || true)"
  fi
  if [[ -z "${current_minio_root_password:-}" ]]; then
    current_minio_root_password="$(read_dev_env_value MINIO_ROOT_PASSWORD || true)"
  fi
  if [[ -z "${current_storage_access_key:-}" ]]; then
    current_storage_access_key="$(read_dev_env_value AWS_ACCESS_KEY_ID || true)"
  fi
  if [[ -z "${current_storage_secret_key:-}" ]]; then
    current_storage_secret_key="$(read_dev_env_value AWS_SECRET_ACCESS_KEY || true)"
  fi
